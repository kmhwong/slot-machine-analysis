...
def run_simulation(config: dict[str, Any], spins: int | None = None, seed: int | None = None) -> dict[str, Any]:
    ...
    free_cfg = config.get("features", {}).get("free_spins", {})
    report["free_spins_retrigger_enabled"] = bool(free_cfg.get("retrigger", True))
    report["max_free_spins_per_round"] = int(free_cfg.get("max_free_spins", 200))
    report["free_spin_multiplier"] = float(free_cfg.get("multiplier", 1.0))
    report["line_win_bet_basis"] = str(free_cfg.get("line_win_bet_basis", "line_bet"))
    report["scatter_win_bet_basis"] = str(free_cfg.get("scatter_win_bet_basis", "total_bet"))
    report["multiplier_applies_to"] = str(free_cfg.get("multiplier_applies_to", "all_wins"))
    report["use_total_bet_multiplier_bug"] = bool(free_cfg.get("use_total_bet_multiplier_bug", False))
    return report
